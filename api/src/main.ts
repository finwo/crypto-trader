...
const strategies = {
    async exchange(settings) {
        ...
        if (account.quote.balance < (settings.target * settings.gap)) {
        ...
        buyOrder.price = new Decimal(settings.target).times(new Decimal(1).minus(settings.gap)).dividedBy(account.base.balance).dividedBy(market.quote_increment).round().times(market.quote_increment).toFixed();
        // buyOrder.price = Math.floor((settings.target * (1 - settings.gap) / account.base.balance) / market.quote_increment) * market.quote_increment;
        buyOrder.size = new Decimal(settings.target).times(new Decimal(1).minus(settings.gap)).times(settings.gap).dividedBy(buyOrder.price).dividedBy(market.base_increment).round().times(market.base_increment).toFixed();
        // buyOrder.size  = Math.floor(((settings.target * (1 - settings.gap) * settings.gap) / buyOrder.price) / market.base_increment ) * market.base_increment;
        ...
        sellOrder.price = new Decimal(settings.target).times(new Decimal(1).plus(settings.gap)).dividedBy(account.base.balance).dividedBy(market.quote_increment).round().times(market.quote_increment).toFixed();
        // sellOrder.price = Math.floor((settings.target * (1 + settings.gap) / account.base.balance) / market.quote_increment) * market.quote_increment;
        sellOrder.size = new Decimal(settings.target).times(new Decimal(1).plus(settings.gap)).times(settings.gap).dividedBy(sellOrder.price).dividedBy(market.base_increment).round().times(market.base_increment).toFixed();
        // sellOrder.size  = Math.floor(((settings.target * (1 + settings.gap) * settings.gap) / sellOrder.price) / market.base_increment ) * market.base_increment;
